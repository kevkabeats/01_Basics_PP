...
output(arr);
output(arr.length); // Länge des Arrays / Anzahl der Elemente
output(arr[0]); // Zugriff auf erstes Element (Index 0)
output(arr[arr.lenght-1]); // Zugriff auf letztes Element (arr.lenght -1) */
...
function getSentenceArr2(arr) 
{
    ...
    /* // 1. Iteration: Stuktur GENAU erzeugen
    for (let i = 0; i < arr.length; i++)
    if (i < arr.length -1)
    {
        str += arr[i] + GAP
